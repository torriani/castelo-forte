 ...
 *
 * Renderiza slides de carrossel + frases de um culto multiplicado no
 * template Casa de Isabel (off-white #efefef, Inter + Playfair italic, logo canto).
 * Zero LLM, 100% mecanico.
 *
 * Uso:
 *   node scripts/render-culto-casa-isabel.mjs <pasta-carrosseis> <frases.json> <pasta-out-img>
 *
 * Exemplo:
 *   node squads/conteudo/scripts/render-culto-casa-isabel.mjs \
 *     outputs/conteudo-cultos/quem-voce-e-em-cristo-07-06/carrosseis \
 *     outputs/conteudo-cultos/quem-voce-e-em-cristo-07-06/frases.json \
 *     apps/conteudo/img/quem-voce-e-em-cristo-07-06
 */
...
const REPO_ROOT = path.resolve(__dirname, '../../..'); // castelo-forte/
...
const LOGO = path.join(REPO_ROOT, 'workspace/businesses/igreja-castelo-forte/brand/logo-castelo-forte-oficial.png');
...
// INTERNA: titulo + divider + sub (texto-midnight, texto-off)
...
function tplCta(intro, marca, data, igreja) {
  return htmlWrap(`
    ...
    .marca{font-family:'Inter',sans-serif;font-weight:700;font-size:96px;line-height:.92;letter-spacing:-3px;color:${TXT};text-transform:uppercase;margin-bottom:24px}
...
function renderSlideHtml(s) {
  switch (s.tipo) {
    ...
    case 'cta':
      return tplCta(
        s.intro || 'Tudo que Deus tem pra te dar comeca em uma unica palavra.',
        s.marca || 'Em Cristo',
        s.data || 'DOMINGOS DE JUNHO',
...
const frases = fs.existsSync(FRASES_JSON) ? JSON.parse(fs.readFileSync(FRASES_JSON, 'utf-8')) : [];

console.log(`\n🎨 Casa de Isabel render`);
console.log(`   ${carrosseis.length} carrosseis × ${carrosseis[0].slides.length} slides`);
